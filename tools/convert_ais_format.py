#!/usr/bin/env python3
"""Convert USCG AIS CSV files to the colav-simulator format.
...
def check_geographic_compatibility(df: pd.DataFrame) -> bool:
    """Check whether input coordinates fit the current Norwegian ENC."""
    lat_min, lat_max = df["LAT"].min(), df["LAT"].max()
    lon_min, lon_max = df["LON"].min(), df["LON"].max()
    print(f"  输入数据地理范围: 纬度 {lat_min:.3f}~{lat_max:.3f}°, 经度 {lon_min:.3f}~{lon_max:.3f}°")
    in_norway = (
        lat_min >= NORWAY_LAT_RANGE[0]
        and lat_max <= NORWAY_LAT_RANGE[1]
        and lon_min >= NORWAY_LON_RANGE[0]
        and lon_max <= NORWAY_LON_RANGE[1]
    )
    if in_norway:
...
def convert(input_path: str | Path, output_path: str | Path) -> None:
    """Convert one AIS CSV file and report geographic compatibility."""
    print(f"\n读取: {input_path}")
