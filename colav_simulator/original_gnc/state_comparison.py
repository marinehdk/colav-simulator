...
def _state_field_contract(module: str, path: tuple, floating: bool) -> tuple[str, FieldRule]:
    """Cache only immutable field metadata; compare every state value on every call."""
    return json.dumps(path, separators=(",", ":")), state_rule(module, path, 0.0 if floating else 0)
...
def compare_state(module: str, expected: dict, actual: dict) -> dict:
    ...
    for path, value in left.items():
        name, rule = _state_field_contract(module, path, isinstance(value, float))
        names[path] = name
        rules[name] = rule
