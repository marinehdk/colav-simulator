...
ALGORITHMS: dict[str, Capability] = {
    "mid_mpc_ipopt": Capability(
        ...
        ("dynamic",),
        "Ship0 raw G3 is established; the scripted targets stay on fixed "
        "constant-course routes, so global all-vessel safety is not established. "
        "Full 1800 s re-verification pending: planner statuses and clearances verified SUCCESS through the 1200 s encounter window.",
    ),
...
VERIFIED_COMBINATIONS: dict[tuple[str, str, str, str], dict[str, Any]] = {
    ...
    ("rule13", "overtaking", "vo", "god"): _evidence(
        role="candidate_g3",
        termination="time_limit",
        minimum_clearance_m=371.685,
        max_heading_delta_deg=95.625,
        max_speed_delta_mps=0.7044,
        solve_count=600,
    ),
    ...
    ("multiship", "paper_ccta2023_multiship", "vo", "god"): _evidence(
        role="candidate_g3",
        termination="time_limit",
        minimum_clearance_m=95.959,
        max_heading_delta_deg=2.5986,
        max_speed_delta_mps=2.1410,
        solve_count=1711,
        target_minimum_clearance_m={
            "Ship1": 95.959,
            "Ship2": 2867.075,
    ...
    ("multiship", "paper_ccta2023_multiship", "potocnik_colreg_fan_mpc", "god"): _evidence(
        role="candidate_g3",
        termination="time_limit",
        minimum_clearance_m=248.9232747639274,
        max_heading_delta_deg=1.7199233827034028,
        max_speed_delta_mps=5.658889999999902,
        solve_count=1998,
        target_minimum_clearance_m={
            "Ship1": 248.9232747639274,
            "Ship2": 266.3113157702991,
            "Ship3": 388.809206335641,
        },
    ),
}
