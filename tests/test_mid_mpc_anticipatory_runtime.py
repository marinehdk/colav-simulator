...
def test_three_target_anticipatory_run_reaches_goal_and_sequences_actions(tmp_path: Path) -> None:
    ...
    try:
        while session.state.value == "RUNNING":
            ...
            if planner["solver_executed"]:
                ...
                maximum_solver_ms = max(maximum_solver_ms, planner["elapsed_ms"])
        ...
        assert maximum_solver_ms < 2000.0, maximum_solver_ms
    finally:
